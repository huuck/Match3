...
namespace King {
	...
	Engine::Engine(const char* assetsDirectory)
		...
		std::string darkness = assets; darkness += "/darkness.png";
		std::string mine = assets; mine += "/mine.png";
		...
		mPimpl->mSdlSurfaceContainer[Engine::TEXTURE_DARKNESS].reset(new SdlSurface(darkness.c_str()));
		mPimpl->mSdlSurfaceContainer[Engine::TEXTURE_MINE].reset(new SdlSurface(mine.c_str()));
	...
	void Engine::Render(Texture texture, float x, float y, float rotation, float scale) {
		glm::mat4 transformation;
		transformation = glm::translate(transformation, glm::vec3(x, y, 0.0f));

		if (scale != 1.0f) {
			transformation = glm::translate(transformation, glm::vec3((1.0f - scale) * GetTextureWidth(texture) / 2.0f, (1.0f - scale) * GetTextureHeight(texture) / 2.0f, 0.0f));
			transformation = glm::scale(transformation, glm::vec3(scale, scale, 1.0f));
		}

		if (rotation) {
			transformation = glm::translate(transformation, glm::vec3(GetTextureWidth(texture) / 2.0f * scale, GetTextureHeight(texture) / 2.0f * scale, 0.0f));
	...
	void Engine::Write(const char* text, float x, float y, float rotation, float scale) {
		glm::mat4 transformation;
		transformation = glm::translate(transformation, glm::vec3(x, y, 0.0f));

		if (scale != 1.0f) {
			transformation = glm::translate(transformation, glm::vec3((1.0f - scale) * CalculateStringWidth(text) / 2.0f, (1.0f - scale) * 10.0f, 0.0f));
			transformation = glm::scale(transformation, glm::vec3(scale, scale, 1.0f));
		}
		//rotation no longer works with custom scale probably :/
		if (rotation) {
			transformation = glm::rotate(transformation, rotation, glm::vec3(0.0f, 0.0f, 1.0f));
