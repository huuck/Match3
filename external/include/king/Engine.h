...
namespace King {
	...
	public:
		...
		void Render(Texture texture, float x, float y, float rotation = 0.0f, float scale = 1.0f);


		float CalculateStringWidth(const char* text) const;
		void Write(const char* text, const glm::mat4& transform);
		void Write(const char* text, float x, float y, float rotation = 0.0f, float scale = 1.0f);
